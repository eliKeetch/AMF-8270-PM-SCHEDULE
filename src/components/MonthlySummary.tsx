  ...
  return (
    <div className="p-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-gray-100 dark:border-slate-800 shadow-sm flex items-center gap-4">
          <div className="bg-blue-100 dark:bg-blue-900/30 p-3 rounded-full text-blue-600 dark:text-blue-400">
            <Clock size={24} />
          </div>
          <div>
            <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Monthly Progress</p>
            <p className="text-2xl font-bold dark:text-white">{overallPercentage}%</p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-gray-100 dark:border-slate-800 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold dark:text-white">{overallCompleted} / {overallTotal}</p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-gray-100 dark:border-slate-800 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Remaining Today</p>
            <p className="text-2xl font-bold dark:text-white">{overallTotal - overallCompleted}</p>
          </div>
        </div>
      </div>

      <h3 className="text-lg font-bold mb-4 dark:text-white">Machine Readiness</h3>
      <div className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
        <table className="w-full text-left">
          <thead className="bg-gray-50 dark:bg-slate-800 border-b border-gray-100 dark:border-slate-700">
            <tr>
              <th className="p-4 font-bold text-sm text-gray-600 dark:text-gray-400">Machine</th>
              <th className="p-4 font-bold text-sm text-gray-600 dark:text-gray-400">Progress</th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
            {stats.sort((a, b) => b.percentage - a.percentage).map((stat) => (
              <tr key={stat.machine.id} className="last:border-0 hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                <td className="p-4 font-bold text-lg dark:text-white">#{stat.machine.number}</td>
                <td className="p-4 w-1/2">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm font-semibold text-gray-600 dark:text-gray-400">{stat.percentage}%</span>
                  </div>
                </td>
                <td className="p-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${
                    stat.percentage === 100 
                      ...
                      : stat.percentage > 0 ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-900/30' : 'bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-gray-500 border border-gray-200 dark:border-slate-700'
                  }`}>
