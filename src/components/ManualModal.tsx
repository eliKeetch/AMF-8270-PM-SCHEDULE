...
export const ManualModal: React.FC<ManualModalProps> = ({ task, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
      <div className={`bg-white dark:bg-slate-900 rounded-3xl overflow-hidden shadow-2xl transition-all duration-300 flex flex-col border border-white/10 ${
        isMaximized ? 'w-full h-full' : 'w-full max-w-6xl h-[90vh]'
      }`}>
        {/* Header */}
        <div className="p-4 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-gray-50 dark:bg-slate-800 shrink-0">
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-xl ${isOil ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400'}`}>
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="p-2 hover:bg-gray-200 dark:hover:bg-slate-700 rounded-xl transition-all text-gray-400 hover:text-gray-900 dark:hover:text-white hidden md:block"
            >
              {isMaximized ? <Minimize2 size={20} /> : <Maximize2 size={20} />}
            </button>
            <button onClick={onClose} className="p-2 hover:bg-gray-200 dark:hover:bg-slate-700 rounded-xl transition-all text-gray-400 hover:text-gray-900 dark:hover:text-white">
              <X size={20} />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          <div className="w-full md:w-80 p-6 bg-gray-50 dark:bg-slate-800 border-r border-gray-100 dark:border-slate-700 overflow-y-auto shrink-0">
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-2xl border border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
              ...
              <a 
                ...
                className="flex items-center justify-center gap-2 bg-gray-900 dark:bg-slate-700 text-white p-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-black dark:hover:bg-slate-600 transition-all shadow-lg"
              >
            ...
            <div className="mt-8 p-4 rounded-2xl bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800">
              <p className="text-[9px] font-bold text-blue-800 dark:text-blue-300 leading-relaxed uppercase">
